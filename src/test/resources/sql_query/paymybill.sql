select ve.BIL_CTM, ve.ORD_NBR, ve.PO_NUM, ap.SRV_TYP, st.ABIL_STS, st.AGR_STS, e.ADR_EMAL, ve.VER_STRT,ATN_1ST, ATN_END
...
where ap.SRV_TYP like 'CATJ%' and st.ABIL_STS in ('B') and st.AGR_STS in ('A') and VER_STRT <= ? and PO_NUM <> '' order by VER_STRT desc